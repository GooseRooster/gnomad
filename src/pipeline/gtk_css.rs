use crate::pipeline::palette::{
    apply_color_map, build_color_map, build_gtk4_define_map, generate_define_color_block,
};
...
pub fn write_gtk_css(scheme: &Scheme) -> Result<()> {
    // GTK3: define block (hyphenated, resolves upstream's derived tokens) +
    // the compact override template with all @var tokens substituted inline.
    let map = build_color_map(scheme);
    let define_block = generate_define_color_block(&map);
    let body = apply_color_map(GTK3_BODY, &map);
    let gtk3_css = format!("{define_block}\n{body}");
    write_css(&gtk3_user_css_path(), &gtk3_css).context("writing gtk-3.0/gtk.css")?;

    // GTK4: write underscore-named @define-color entries (libadwaita's own
    // token convention) to a separate gnomad-colors.css file.
    ...
    let gtk4_map = build_gtk4_define_map(scheme);
    let colors_css = generate_define_color_block(&gtk4_map);
