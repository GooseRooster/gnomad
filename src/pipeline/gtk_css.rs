...
pub fn write_gtk_css(scheme: &Scheme) -> Result<()> {
    ...
    let gtk3_css = apply_color_map(GTK3_BODY, &map);
    let gtk3_path = gtk3_user_css_path();
    write_css(&gtk3_path, &gtk3_css).context("writing gtk-3.0/gtk.css")?;

    // GTK4: only the @define-color block (libadwaita handles widget styling)
    let gtk4_css = generate_define_color_block(&map);
